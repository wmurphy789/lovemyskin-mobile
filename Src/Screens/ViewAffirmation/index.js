...
const ViewAffirmation = ({ navigation, route }) => {
    ...
    const DummyMusicBar = () => (       // dummy UI of Music SeekBar
        <View style={{ flexDirection: "row", marginTop: 15, alignItems: 'center', marginHorizontal: 25 }}>
            <View style={{ flex: 0.4, height: 5, backgroundColor: "#fff", borderRadius: 10 }} />
            <View style={{ flex: 0.6, height: 2, backgroundColor: "#fff", borderRadius: 10 }} />
        </View>
    )
    ...
    const Header = () => (
        <View style={styles.headerContainer}>
            <Image resizeMode='stretch' style={styles.curveHeaderImage} source={AppImages.curveLightGreenHeaderImage} />
            <TouchableOpacity style={styles.curveHeaderIcon} onPress={() => { goBack() }}>
                <Image
                    source={AppImages.backWhite}
                    style={styles.curveHeaderIcon} />
            </TouchableOpacity>
            <TouchableOpacity activeOpacity={0.8}>
                <ImageBackground style={styles.addMyAffirmation} source={AppImages.gradientButtonImage}  >
                    <Image source={AppImages.editWhiteIcon}
                        resizeMode='contain'
    ...
    const CustomIconButton = ({ icon, onPress, customStyles }) => (
        <TouchableOpacity activeOpacity={0.5} style={[styles.itemImage, customStyles]} onPress={onPress}>
    ...
    const _renderAffirmations = ({ item, index }) => (
        <View style={[styles.ItemView, { backgroundColor: screenColor, marginBottom: index == screenData.length - 1 ? 0 : responsiveHeight(2) }]}>
            <Text style={styles.itemTitle}>{item.title}</Text>
            <Text style={styles.itemtext}>{item.content}</Text>
            <CustomIconButton icon={AppImages.shareBlackIcon} onPress={() => { alert("Share!") }} />
        </View>
    )
        ...
        <View style={[styles.ItemView, { backgroundColor: screenColor, marginBottom: index == screenData.length - 1 ? 0 : responsiveHeight(2) }]}>
            <View style={{ flexDirection: "row", justifyContent: 'space-between', }}>
                ...
                <View>
                    ...
                    <CustomIconButton icon={AppImages.deleteBlackIcon} customStyles={styles.customStylesDeleteButton} />
                </View>
            </View>
            <View >
                <Text style={styles.itemtext}>{item.content}</Text>
                {item.containsMusic && <View >
                    <DummyMusicBar />
                    ...
                    <CustomIconButton
                        icon={isPlaying ? AppImages.pauseWhiteIcon : AppImages.playWhiteIcon}
